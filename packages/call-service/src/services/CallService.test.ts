...
import { checkWaitSLA, checkHoldLimit, checkShortCall, checkReroute } from '../domain/rules';
...
describe('resolveNextStatus', () => {
  ...
  it('throws InvalidTransitionError for illegal transition', () => {
    expect(() => resolveNextStatus('waiting', 'call_hold')).toThrow(InvalidTransitionError);
  });

  it('throws InvalidTransitionError when transitioning from ended', () => {
    expect(() => resolveNextStatus('ended', 'call_answered')).toThrow(InvalidTransitionError);
  });
});
...
function makeEventRepo(overrides: Partial<CallEventRepository> = {}): CallEventRepository {
  return {
...
function makeExistingCall(): Call {
  return new Call('call-1', 'voice', 'waiting', 'medical_english', new Date(), undefined);
}
...
describe('CallService', () => {
  ...
  describe('processEvent: call_initiated', () => {
    it('creates a call with status waiting and records the event', async () => {
      ...
      expect(callRepo.createCall).toHaveBeenCalledWith(
        expect.objectContaining({ id: 'call-1', status: 'waiting', type: 'voice' }),
      );
    ...
    it('is idempotent: returns the existing event on duplicate call_initiated', async () => {
      const existingEvent = new CallEvent('evt-1', 'call-1', 'call_initiated', new Date());
      eventRepo = makeEventRepo({
  ...
  describe('processEvent: call_answered', () => {
    it('updates call status to active', async () => {
      callRepo = makeCallRepo({ findCallById: vi.fn().mockResolvedValue(makeExistingCall()) });
      service = new CallService(callRepo, eventRepo);

      await service.processEvent({ event: 'call_answered', callId: 'call-1', waitTime: 10 });

      expect(callRepo.updateCallStatus).toHaveBeenCalledWith('call-1', 'active');
    });
    ...
    it('includes wait_sla_exceeded flag when waitTime exceeds 30s', async () => {
      ...
      await service.processEvent({ event: 'call_answered', callId: 'call-1', waitTime: 35 });

      expect(eventRepo.recordEvent).toHaveBeenCalledWith(
        expect.objectContaining({
          metadata: expect.objectContaining({
            flags: expect.arrayContaining([expect.objectContaining({ rule: 'wait_sla_exceeded' })]),
          }),
        }),
      );
    });
    ...
    it('throws NotFoundError for an unknown callId', async () => {
      await expect(
        service.processEvent({ event: 'call_answered', callId: 'missing', waitTime: 5 }),
  ...
  describe('processEvent: call_hold', () => {
    it('updates call status to on_hold', async () => {
      callRepo = makeCallRepo({
        findCallById: vi.fn().mockResolvedValue(new Call('call-1', 'voice', 'active', 'medical_english', new Date())),
      });
      service = new CallService(callRepo, eventRepo);

      await service.processEvent({ event: 'call_hold', callId: 'call-1', holdDuration: 30 });

      expect(callRepo.updateCallStatus).toHaveBeenCalledWith('call-1', 'on_hold');
    });
    ...
    it('includes hold_limit_exceeded flag when holdDuration exceeds 60s', async () => {
      ...
      await service.processEvent({ event: 'call_hold', callId: 'call-1', holdDuration: 90 });

      expect(eventRepo.recordEvent).toHaveBeenCalledWith(
        expect.objectContaining({
          metadata: expect.objectContaining({
            flags: expect.arrayContaining([expect.objectContaining({ rule: 'hold_limit_exceeded' })]),
          }),
        }),
      );
    });
  ...
  describe('processEvent: call_ended', () => {
    it('updates call status to ended with endTime', async () => {
      ...
      await service.processEvent({ event: 'call_ended', callId: 'call-1', endReason: 'completed', duration: 60 });

      expect(callRepo.updateCallStatus).toHaveBeenCalledWith('call-1', 'ended', expect.any(Date));
    });
    ...
    it('includes short_call flag when duration is under 10s', async () => {
      ...
      await service.processEvent({ event: 'call_ended', callId: 'call-1', endReason: 'abandoned', duration: 5 });

      expect(eventRepo.recordEvent).toHaveBeenCalledWith(
        expect.objectContaining({
          metadata: expect.objectContaining({
            flags: expect.arrayContaining([expect.objectContaining({ rule: 'short_call' })]),
          }),
        }),
      );
    });
  ...
  describe('invalid state transitions', () => {
    it('throws InvalidTransitionError when call_hold is applied to a waiting call', async () => {
      ...
      await expect(
        service.processEvent({ event: 'call_hold', callId: 'call-1', holdDuration: 30 }),
  ...
  describe('getCalls', () => {
    it('delegates to callRepo.listCalls', async () => {
      ...
      callRepo = makeCallRepo({ listCalls: vi.fn().mockResolvedValue(mockCalls) });
      service = new CallService(callRepo, eventRepo);
  ...
  describe('getCallEvents', () => {
    it('delegates to eventRepo.listEventsForCall', async () => {
      const mockEvents = [new CallEvent('e1', 'call-1', 'call_initiated', new Date())];
      eventRepo = makeEventRepo({ listEventsForCall: vi.fn().mockResolvedValue(mockEvents) });
      service = new CallService(callRepo, eventRepo);
